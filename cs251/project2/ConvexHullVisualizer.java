...
public class ConvexHullVisualizer {
  ...
  */
  ...
  {       
      ...
      ConvexHullVisualizer hull = new ConvexHullVisualizer(size);
      ...
      for(int i = 2; i<hull.currentNumberOfPoints; i++){
          if(hull.points.get(i).x > maxX)
              maxX = hull.points.get(i).x;
          if(hull.points.get(i).y > maxY)
              maxY = hull.points.get(i).y;
      ...
      ArrayList<Point> unitList = new ArrayList<Point>();
      ArrayList<Point> unitSolution = new ArrayList<Point>();
      StdDraw.setPenColor(StdDraw.WHITE);
      StdDraw.clear(StdDraw.BLACK);
      //For loop to map all coordinates to unit scale
      for(int x = 0; x < hull.currentNumberOfPoints; x++)
      {
          Point unit = new Point(hull.points.get(x).x/maxX, hull.points.get(x).y/maxY, hull.points.get(x).index);
      ...
      {
          lower.get(x).print();
          Point unit = new Point(lower.get(x).x/maxX, lower.get(x).y/maxY, lower.get(x).index);
          Point unitTarget = new Point(lower.get((x+1)%lower.size()).x/maxX, lower.get((x+1)%lower.size()).y/maxY,lower.get((x+1)%lower.size()).index);
